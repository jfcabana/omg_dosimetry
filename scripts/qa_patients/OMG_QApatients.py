...
#%% ################################## Entrer vos informations ci-dessous ##################################
    #### Infos film ####
    info = dict(author = 'DS',                 # Initiales du phys qui a fait la mesure
                unit = '1',                     # Salle
                film_lot = 'C14',               # Lot de film utilisé. Important car détermine la calibration à prendre.
                scanner_id = 'Epson 12000XL',   # Ne change pas
                date_exposed = '2023-05-02',    # Date d'exposition des films
                date_scanned = '2023-05-03',    # Date de numérisation des films
                wait_time = '16h',              # Temps d'attente entre l'exposition et la numérisation
                notes = 'Scan en transmission ') 

    #### Infos plan ####
    ID_patient = 'phy_sbrtoligo_e2e'         # ID du patient
    ID_plan = 'b1b'           # ID du plan
    prescription = 700           # Dose prescription (cGy / fraction)
...
#    fantome = 'output electrons'       # Fantôme pour output électrons. Utiliser lorsque mesure film électrons préparé avec script QA électrons.
    ...
    norm_isodose = prescription * 1.2  # Dose [cGy] à utiliser pour la normalisation de type 'isodose'
...
#    elif info['film_lot'] == 'C14': lut_file = os.path.join(path_calib, 'C14_calib_18h_trans_300ppp_0-9Gy.pkl')
    ...
    if fantome == 'autre':
        ...
        #markers_center = [0.5, -345.0, 254.6]     #sbrtoligo a1a 
        #markers_center = [2, -344, 255.5]     #sbrtoligo a1a 
        markers_center = [4.4, -269.0, 259.5]     #sbrtoligo b1a 
...
#%% ################################## NE RIEN CHANGER PLUS BAS ########################################
    ...
    if dose_2_analysis:
        ...
        film.gamma_analysis(doseTA=doseTA_1, distTA=distTA_1, threshold=threshold, norm_val=norm_val, film_filt=film_filt, max_gamma=max_gamma, random_subset=random_subset)
        print("\n===================== Analyse Gamma ========================")
        ...
        film.gamma_analysis(doseTA=doseTA_2, distTA=distTA_2, threshold=threshold, norm_val=norm_val, film_filt=film_filt, max_gamma=max_gamma, random_subset=random_subset)
        print("\n===================== Analyse Gamma ========================")
