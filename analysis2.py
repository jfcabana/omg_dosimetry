...
class DoseAnalysis():
    ...
    def __init__(self, film_dose=None, ref_dose=None, film_dose_factor=1, ref_dose_factor=1, flipLR=False, flipUD=False, rot90=0, ref_dose_sum=False, apply_factor_from_roi=False):
    ...
    def apply_factor_from_roi(self):
        ...
        ax.set_title('Click and drag to draw an ROI manually. Press ''enter'' when finished.')
        print('Click and drag to draw an ROI manually. Press ''enter'' when finished.')
        ...
        self.cid = self.fig.canvas.mpl_connect('key_press_event', self.press_enter)
...
#            plt.close(plt.gcf()) 
            ...
            self.factor_from_roi = roi_ref/roi_film
            self.film_dose_factor = self.factor_from_roi
            self.film_dose.array = self.factor_from_roi * self.film_dose.array
            
            print("Applied factor from ROI = {}".format(self.factor_from_roi))
    ...
    def computeGamma(self, doseTA=2, distTA=2, threshold=0.1, norm_val=None):
        """Adapted from pylinac image module gamma method
...
#       # error checking
        if not is_close(self.film_dose.dpi, self.ref_dose.dpi, delta=0.3):
        ...
        ref_img.array[ref_img.array < threshold * np.max(ref_dose)] = np.NaN
        ...
        img_x = spf.sobel(ref_dose.as_type(np.float32), 1)
        img_y = spf.sobel(ref_dose.as_type(np.float32), 0)
        grad_img = np.hypot(img_x, img_y)
        
        # equation: |measurement - reference| / sqrt ( doseTA^2 + distTA^2 * image_gradient^2 )
        subtracted_img = np.abs(ref_dose - ref_img)
        denominator = np.sqrt((doseTA / 100.0 ** 2) + ((distTA_pixels ** 2) * (grad_img ** 2)))
        gamma_map = subtracted_img / denominator

        GammaMap = imageRGB.ArrayImage(gamma_map, dpi=self.film_dose.dpi)
        GammaMap.subtracted_img = imageRGB.ArrayImage(subtracted_img, dpi=self.film_dose.dpi)
        GammaMap.grad_img = imageRGB.ArrayImage(grad_img, dpi=self.film_dose.dpi)
        
        
        fail = np.zeros(GammaMap.shape)
        fail[(GammaMap.array > 1.0)] = 1
        GammaMap.fail = imageRGB.ArrayImage(fail, dpi=self.film_dose.dpi)
        
        passed = np.zeros(GammaMap.shape)
        passed[(GammaMap.array <= 1.0)] = 1
        GammaMap.passed = imageRGB.ArrayImage(passed, dpi=self.film_dose.dpi)
...
#       # error checking
    ...
    def show_results(self, fig=None, x=None, y=None):
        ...
        max_dose_film = np.percentile(self.film_dose.array,[99.9])[0].round(decimals=-1)
        ...
        min_value = np.percentile(self.DiffMap.array,[1])[0].round(decimals=0)
        max_value = np.percentile(self.DiffMap.array,[99])[0].round(decimals=0)
