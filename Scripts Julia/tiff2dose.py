...
class Gaf:
    ...
    def __init__(self, path='', lut_file='', img_filt=0, lut_filt=35, fit_type='rational', k=3, ext=3, s=0, info=None, crop_edges=0):
...
#            Dm,Dr,Dg,Db,Ar,Ag,Ab = GetDoses(row, p_lut)
    ...
    def show_results(self):
        max_dose_m = np.percentile(self.dose_m.array,[99])[0].round(decimals=-1)
        max_dose_opt = np.percentile(self.dose_opt.array,[99])[0].round(decimals=-1)
        max_dose_cst = np.percentile(self.dose_consistency.array,[99])[0].round(decimals=-1)
        clim = [0, max(max_dose_m, max_dose_opt)]
        clim_cst = [0,max_dose_cst]

        self.figure, ((ax1,ax2,ax3),(ax4,ax5,ax6),(ax7,ax8,ax9)) = plt.subplots(3,3,figsize=(14, 9))

        self.dose_r.plotCB(ax1,clim=clim, title='Red channel dose', show=False)
        self.dose_g.plotCB(ax2,clim=clim, title='Green channel dose', show=False)
        self.dose_b.plotCB(ax3,clim=clim, title='Blue channel dose', show=False)
        self.dose_m.plotCB(ax4,clim=clim, title='Mean channel dose', show=False)
        self.dose_rg.plotCB(ax5,clim=clim, title='Red+Green Average dose', show=False)
        self.dose_consistency.plotCB(ax6,clim=clim_cst, cmap='gray', title='Consistency', show=False)
        self.dose_opt.plotCB(ax7,clim=clim, title='Multichannel optimized dose', show=False)
        self.dose_opt_delta.plotCB(ax8, cmap='gray', title='Disturbance', show=False)
        self.dose_opt_RE.plotCB(ax9, cmap='gray', title='Residuals', show=False)

        self.figure.tight_layout()
        self.figure.canvas.set_window_title('Tiff to Dose Results')
        plt.show()
    ...
    def publish_pdf(self, filename=None, author=None, unit=None, notes=None, open_file=False):
        ...
        text = ['Film file: {}'.format(self.path),
                'LUT file: {}'.format(self.lut_file),
