...
#        img1 = imageio.imread(file_in)
#        img1.meta.resolution = (72,72)
...
class DicomImage(BaseImage):
    ...
    def __init__(self, path, *, dtype=None, dpi=None, sid=None, doseUnits='cGy', doseScalingFactor=None):
        ...
        print(self.metadata['0x20','0x32'].value)
        # read a second time to get pixel data
        if isinstance(path, BytesIO):
            path.seek(0)
        ds = pydicom.read_file(path, force=True)
        if dtype is not None:
            self.array = float(self.metadata.DoseGridScaling)*ds.pixel_array.astype(dtype)
        else:
            self.array = float(self.metadata.DoseGridScaling)*ds.pixel_array
    ...
    def mutliply(self, value):

        return [[x * N for x in y] for y in hh]
...
#                self.array = np.array(pil_image)
            if array.ndim > 2 and array.dtype == 'uint16':              # check if this is a 16-bit RGB image
