...
class LUT:
    ...
    def __init__(self,
        ...
        self.roi_crop_width = roi_crop_width
        self.roi_crop_length = roi_crop_length
        self.info = info
        ...
        self.figure = None              # store the figures
    ...
    def detect_film(self):
        ...
        if self.roi_size == 'auto':
            ...
            self.roi_width = (np.floor(np.asarray(width) - self.roi_crop_width*2 * self.img.dpmm)).astype('int')
            self.roi_length = (np.floor(np.asarray(length) -  self.roi_crop_length*2 * self.img.dpmm)).astype('int')
    ...
    def select_film(self):
        ...
        fig = plt.gcf()
        fig.canvas.set_window_title('ROIs Selection')
    ...
    def create_LUT(self):
        ...
        if nDose != len(self.doses):
            raise ValueError("Number of films does not match number of doses! Number of deteted films:", nDose)
    ...
    def plot_roi(self, ax=None):      
        ...
        fig = plt.gcf()
        fig.canvas.set_window_title('Calibration Results')
    ...
    def save_calibration_curves(self, outputfile="calibration.txt"):

#        print(self.y_data)

        with open(outputfile, 'w+') as file:
            file.write("Dose R G B \n")
            for x in range(len(self.x_data)):
                file.write("{} {} {} {} \n".format(self.x_data[x], self.y_data[0][x],self.y_data[1][x], self.y_data[2][x]))

    def plot_fit(self, ax=None, i=None, show_derivative=False, fit_type='rational'):
        colors = ['k','r','g','b']
        if i is None:
        ...
        if ax is None:
            ...
            ax1 = ax
        self.y_data = []
        mindata =min(np.min(self.channel_R), np.min(self.channel_G), np.min(self.channel_B))
        maxdata =max(np.max(self.channel_R), np.max(self.channel_G), np.max(self.channel_B))
        self.x_data = np.arange(mindata, maxdata, 0.001)
        ...
        for j in range(2,6):
            if j !=1:
                    ...
                    if show_derivative:
                        if fit_type == 'rational':
                            y, yd = self.get_dose_and_derivative_from_fit(xdata, ydata, self.x_data)
                        elif fit_type == 'spline':
                            y, yd = self.get_dose_and_derivative_from_spline(xdata, ydata, self.x_data)
                        ax1.plot(ydata,xdata,'o',color=colors[j-2])
                        ax1.plot(y,x,color=colors[j-2])
                        ax2.plot(yd,x,color=colors[j-2])
                    else:
                        if fit_type == 'rational':
                            y_temp=self.get_dose_from_fit(xdata, ydata, self.x_data)
                            y_temp[y_temp>np.max(self.doses_corr)]= np.nan
                            y_temp[y_temp<0]= np.nan
                            self.y_data.append(y_temp)
                            ax1.plot(ydata,xdata,'o',color=colors[j-2])
                        elif fit_type == 'spline':
                            self.y_data.append( self.get_dose_from_spline(xdata, ydata, self.x_data))
                            # np.savetxt(file, y, delimiter=',')
                            ax1.plot(ydata,xdata,'o',color=colors[j-2])
                        ax1.plot(self.y_data[j-2],self.x_data,color=colors[j-2])


    def show_results(self, fit_type='rational'):
...
#            ax6 = plt.subplot2grid((4, 6), (3, 4), colspan=2)
            ...
            self.plot_fit(i='mean', ax=ax3, fit_type=fit_type)
            ...
            self.plot_roi(ax=ax1)
            self.plot_profile(ax=ax2)
            self.plot_fit(ax=ax3, fit_type=fit_type)
    ...
    def save_analyzed_image(self, filename, fit_type='rational', **kwargs):
        ...
        kwargs
            Keyword arguments are passed to savefig().
        """
        self.show_results(fit_type=fit_type)
        self.figure.savefig(filename)
        plt.close(self.figure)
            
            
    def publish_pdf(self, filename=None, fit_type='rational', author=None, unit=None, notes=None, open_file=False):
        ...
        data = io.BytesIO()
        self.save_analyzed_image(data, fit_type=fit_type)
        ...
        pdf.draw_text(canvas, x=1 * cm, y=21.5 * cm, text='Calibration options:', fontsize=12)
...
def get_profile(file):
    ...
    with open(file, 'r') as f:
      ...
      content = list(reader)    
    string = ['Detector ID', 'Negative Diagonal Position(cm)', 'Set 1']
    line = content.index(string)
    profile = np.empty((len(content[line+1:]), 2))  
    for i in range(len(content[line+1:])):
        profile[i,0] = float(content[i+line+1][1].replace(',','.')) * 10 # Change from cm to mm 
